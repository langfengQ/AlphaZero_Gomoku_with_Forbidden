...
class TrainPipeline():
    def __init__(self, init_model=None,transfer_model=None):
        ...
        if rank in range(0,7):
            os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
            os.environ["CUDA_VISIBLE_DEVICES"] = "3"
            cuda = True
        elif rank in range(7,31):
            cuda = True
            os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
            os.environ["CUDA_VISIBLE_DEVICES"] = "2"
        elif rank in range(31,36):
            cuda = True
            os.environ["CUDA_DEVICE_ORDER"] = "PCI_BUS_ID"
        ...
        try:
            for num in range(self.game_batch_num):
                ...
                if rank ==0:
                    ...
                    now_lr_time = (time.time()-start_time) / 3600
                    if now_lr_time > 280 and self.learn_rate > 8e-4:
                        self.learn_rate = self.learn_rate * 0.1
                    # print train epoch and total game num
                    print('-' * 50 + 'rank: {}, train epoch :{},total game :{}, lr: {}'.format(rank, num, self.game_count, self.learn_rate))
...
if __name__ == '__main__':
    ...
    # training_pipeline = TrainPipeline(init_model=None, transfer_model='model_11_11_5/best_policy.model')
