...
class TreeNode(object):
    ...
    def expand(self, action_priors,add_noise):
        ...
        if add_noise:
            ...
            dirichlet_noise = np.random.dirichlet(0.15 * np.ones(length))
...
class MCTSPlayer(object):
    ...
    def __init__(self, policy_value_function,action_fc,evaluation_fc,c_puct=5, n_playout=400, is_selfplay=0):
        ...
        self.first_n_moves = 8
        self.second_n_moves = 12
    ...
    def get_action(self,board,is_selfplay,print_probs_value):
        ...
        if len(sensible_moves) > 0:
            if is_selfplay:
                ...
                step = board.width * board.height - len(board.availables)
                if step <= self.first_n_moves:
                    # temp = 0.6 if step % 2 == 0 else 0.4  # tau of black is 0.8, tau of white is 0.4
                    temp = 0.8
                    # For the first n moves of each game, the temperature is set to τ = 1
                    probs = softmax(1.0 / temp * np.log(np.array(visits) + 1e-10))
                    move = np.random.choice(acts, p=probs)
                elif step > self.first_n_moves and step <= self.second_n_moves:
                    # temp = 0.6 if step % 2 == 0 else 0.4  # tau of black is 0.8, tau of white is 0.4
                    temp = 0.6
                    # For the first n moves of each game, the temperature is set to τ = 1
                    probs = softmax(1.0 / temp * np.log(np.array(visits) + 1e-10))
